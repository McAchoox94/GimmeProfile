...
require (XCRUD_PATH . '../../config.php'); // database class
Xcrud_config::$dbname = DATABASE;
Xcrud_config::$dbuser = USERNAME;
Xcrud_config::$dbpass = PASSWORD;
Xcrud_config::$dbhost = HOST;
...
class Xcrud
{
        ...
        foreach ($fdata as $fitem)
        {
            ...
            if ($mode == 'all' or $mode == 'create')
                $this->alert_create[] = array(
                    'column' => $fitem ? $fitem['table'] . '.' . $fitem['field'] : false,
                    'cc' => $cc,
                    'subject' => $subject,
                    'message' => $message,
                    'link' => $link,
    ...
    protected function _entry($mode = 'edit', $postdata = array())
    {
        ...
        if ($this->{$callback_method})
        {
            ...
            {
                $postdata = new Xcrud_postdata($this->result_row, $this);
                call_user_func_array($this->{$callback_method}['callable'], array($postdata, $this));
                $this->result_row = $postdata->to_array();
            }
        }
        ...
        if (!$this->primary_val)
        {
            ...
            if ($this->alert_create)
            {
                foreach ($this->alert_create as $alert)
                {
                    ...
                    $send_to = isset($postdata[$alert['column']]) ? isset($postdata[$alert['column']]) : isset($postdata[$alert['column']]);
